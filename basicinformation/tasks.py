...
CQ_KEY ='AIzaSyAuVg3prMt56bhxuDbv4mna2xUw05h2ciA'
...
def add_to_database_questions(sheet_link,school,production=False,onlyImage =
        ...
        for sh in sheet_link:
            if production:
                df=\
                pd.read_csv('/home/ubuntu/bodhiai/question_data/csv/'+sh,error_bad_lines=False )
            else:
                df=\
                pd.read_csv('/home/ubuntu/bodhiai/question_data/csv/'+sh,error_bad_lines=False )
            ...
            for ind in range(len(optA)):
                if optE[ind] == 'E':
                    if onlyImage:
                        write_questions(school,None,optA[ind],optB[ind],optC[ind],optD[ind],optE[ind],images[ind],right_answer[ind],quest_category[ind],exp[ind],sectionType[ind],str(lang[ind]),used_for[ind],source[ind],fouroptions='5',direction = direction[ind],difficulty= difficulty[ind] )
                else:

                #if onlyImage:
                    write_questions(school,None,optA[ind],optB[ind],optC[ind],optD[ind],None,images[ind],right_answer[ind],quest_category[ind],exp[ind],sectionType[ind],str(lang[ind]),used_for[ind],source[ind],fouroptions='4',direction = direction[ind],difficulty= difficulty[ind] )
                counter_quest += 1
                print('this is question number {}'.format(counter_quest+1))
                #else:
...
write_questions(school,question,optA,optB,optC,optD,optE,image,correctOpt,questCategory,exp,sectionType,lang,used_for,source,fouroptions,direction=False,replace=False,difficulty=None):
    ...
    print('difficulty {}, sectionType {}'.format(difficulty,sectionType))
    print('correct option {}, image {}'.format(correctOpt,image))
    try:
        ...
        if replace:
            ...
            elif sectionType.strip() == 'Chemistry_IIT':
                new_questions.section_category = 'Chemistry_IIT'
            elif sectionType.strip() == 'chemistry_iit':
                new_questions.section_category = 'Chemistry_IIT'

            elif sectionType.strip() == 'Maths_IIT':
                new_questions.section_category = 'Maths_IIT'
            elif sectionType.strip() == 'Physics_JEE':
                new_questions.section_category = 'Physics_IIT'
            elif sectionType.strip() == 'Physics_NEET':
                new_questions.section_category = 'Physics_NEET'

            elif sectionType.strip() == 'Chemistry_NEET':
                new_questions.section_category = 'Chemistry_NEET'
            elif sectionType.strip() == 'physics_neet':
                new_questions.section_category = 'Physics_NEET'
            elif sectionType.strip() == 'bio':
                new_questions.section_category = 'Biology_NEET'
            ...
            if difficulty:
                print('in difficulty {}'.format(difficulty))
                if str(difficulty) == 'E':
                    new_questions.diffculty_category = 'easy'
                elif str(difficulty) == 'M':
                    new_questions.diffculty_category = 'medium'
                elif str(difficulty) == 'D':
                    new_questions.diffculty_category = 'hard'
            ...
            if direction:
                if type(direction) == str:
                    try:
                        ...
                        print("--comprehension error {}".format(str(e)))
...
def delete_allQuestions(school):
    questions =\
    SSCquestions.objects.filter(section_category= 'Quantitative-Analysis')
    for i in questions:
        i.delete()
...
def add_questions(institute,section):
    ...
    if institute == 'JECRC':
       ...
       for i in questions:
           i.school.add(school)
    elif institute == 'Competition_Qualifiers':
       questions = SSCquestions.objects.filter(school__name =
                                               'BodhiAI',section_category=section)
       school = School.objects.get(name = institute)
       print('%s --num quests' %len(questions))
    ...
    try:
        ...
        try:
            ...
            for i in response['Items']:
                student_id = i['ranking']
                all_students.append(student_id)
            all_ids = []
            for i in all_students:
                for where,j in enumerate(i):
                    try:
                        val = int(j)
                    except:
                        ind = where
                        all_ids.append(i[:ind])
                        break
            for index,i in enumerate(all_ids):
                print('change rank notification {} -- {}'.format(index,i))
                student_id = int(i)
                student_obj = Student.objects.get(id = student_id)
                student_token = StudentExpoToken.objects.get(student =
                                                             student_obj)
                student_id = all_students[index]
                ...
                if ranking[-1] > ranking[-2]:
                    message =\
            """Your ranking has gone down from {} to {}. Take more tests to practice and \
                improve your ranking.""".format(str(ranking[-1]),str(ranking[-2]))
                title = "Ranking gone down. Do something! Take a test."
                custom_data = {'activity':'subjectRank','id':'subject'}
                sendExpoNotification2.delay(student_token.token,message,title,custom_data)
    ...
    try:
        ...
        try:
            ...
            """Your ranking has gone down from {} to {}. Take more tests to practice and \
                improve your ranking.""".format(str(ranking[-1]),str(ranking[-2]))
                title = "Ranking gone down. Do something! Take a test."
                in_id = {'subject':subject,'chapter':chapter_code}
                custom_data = {'activity':'chapterRank','id':in_id}
                sendExpoNotification2.delay(student_token.token,message,title,custom_data)
...
def delete_questions():
    ...
    print('{} num questions to be deleted'.format(len(quests)))
    for i in quests:
        if i.source == 'ssc':
            i.delete()
...
def delete_directions():
    directions = Comprehension.objects.all()
    for i in directions:
        if i.picture == 'nan':
            i.delete()
...
def delete_nan_text():
    quests = SSCquestions.objects.all()
    for i in quests:
        if i.text == 'nan' or i.text.startswith('https'):
            new_text = ''
            i.text = new_text
            i.save()
...
@shared_task
def\
sendExpoNotification(token,message,title):
    extra = None
    try:
...
@shared_task
def sendExpoNotification2(token,message,title,custom_data=None):
    extra = custom_data
    print('notification data {}'.format(extra))

    try:
        data_send = json.dumps(extra)
        r = requests.post('https://exp.host/--/api/v2/push/send',data={"to":  token,
                                                                       "title":title,
                                                                       "body": message,
                                                                       "sound":"default",
                                                                       "priority":"high",
                                                                        "channelId":"notifyChannel_1",
                                                                       "data":data_send})
        print('notification code {}'.format(r.status_code))
    except Exception as e:
        print(str(e))
        r = requests.post('https://exp.host/--/api/v2/push/send',data={"to":  token,"title":title,
                                                                       "body": message,
                                                                       "sound":"default",
                                                                       "priority":"high",
                                                                       "channelId":"notifyChannel_1"})
        print('notification code except {}'.format(r.status_code))

@shared_task
def automatic_test_creation(subject,questions_in_test):
    print('creating test of {}'.format(subject))
    subject_chapters = SubjectChapters.objects.filter(subject=subject)
    for chap in subject_chapters:
        print('creating test of chapter {}'.format(chap.name))
        combined_name = '{}_{}'.format(subject,chap.name)
        print('this is combined name {}'.format(combined_name))
        prev_tests = SSCKlassTest.objects.filter(name=combined_name,klas__name
                                                 ='NEET')
        if len(prev_tests) < 3:
            code = chap.code
            final_questions = []
            questions_obj = SSCquestions.objects.filter(section_category =
                                                        subject,topic_category =
                                                        str(code))
            for qo in questions_obj:
                num_used = 0
                if qo not in final_questions:
                    prev_tests = SSCKlassTest.objects.all()
                    for pt in prev_tests:
                        prev_questions = pt.sscquestions_set.all()
                        for pq in prev_questions:
                            if pq == qo:
                                num_used += 1
                                continue
                if num_used == 0:
                    final_questions.append(qo)
                    if len(final_questions) == questions_in_test:
                        new_test = SSCKlassTest()
                        test_name = "{}_{}".format(subject,chap.name)
                        new_test.name = test_name
                        kl = klass.objects.get(school__name =
                                               'BodhiAI',name='NEET')
                        date = timezone.now()
                        new_test.klas = kl
                        teacher = Teacher.objects.get(name='BodhiAI')
                        new_test.creator = teacher.teacheruser
                        new_test.sub = subject
                        new_test.published = date
                        test_marks = 0
                        for qu_mar in final_questions:
                            test_marks += qu_mar.max_marks

                        new_test.totalTime = 15
                        new_test.max_marks = test_marks
                        new_test.save()
                        print('New test saved')
                        for qu in final_questions:
                            qu.ktest.add(new_test)

                        for added_qu in new_test.sscquestions_set.all():
                            timesus = TimesUsed.objects.filter(teacher =
                                              teacher,quest =
                                              added_qu,batch = kl)
                            if len(timesus) == 1:
                                ...
                                tused = TimesUsed()
                                tused.numUsed = 1
                                tused.teacher = teacher
                                tused.quest = added_qu
                                tused.batch = kl
                                tused.save()

        
                        test_details = TestDetails()
                        test_details.test = new_test
                        test_details.num_questions = len(final_questions)
                        id_final_questions = []
                        for i in final_questions:
                            id_final_questions.append(i.id)
                        test_details.questions = id_final_questions
                        test_details.typeTest = 'chapterwise'
                        test_details.save()

                        students = Student.objects.filter(klass =\
                                                          kl,school__name='BodhiAI')
                        tokens = []
                        for i in students:
                            new_test.testTakers.add(i)
                            new_test.save()
                            try:
                                stud_token = StudentExpoToken.objects.get(student=i)
                                tokens.append(stud_token.token)
                            except Exception as e:
                                print(str(e))


       
                        new_test.mode = 'BodhiOnline'
                        new_test.save()
                        for i in tokens:
                            messageTo = 'Congrats ! Bodhi AI has a new test for you in {}'.format(new_test.sub)
                            title = "New Test !!"
                            try:
                                sendExpoNotification2(i,messageTo,title)
                                #sendExpoNotification.delay(i,messageTo,title)
                                print('message sent')
                            except Exception as e:
                                print(str(e))
...
@shared_task
def TeacherGoLiveNotification(title,body,school,batch):
    if 'JEN' in school:
        sender_token = JEN_KEY
    elif 'Competition_Qualifiers' in school:
        sender_token = CQ_KEY
        print('using cq key')
    else:
        sender_token = BODHIAI_KEY
    ...
    link = "https://fcm.googleapis.com/fcm/send"
    response = requests.post(link,data=json.dumps(payload),headers=headers)
    print('{} response code live video'.format(response.status_code))
    return response.status_code
...
@shared_task
def edit_wrong_question(sheet_link):
    print('in wrong edit question {}'.format(sheet_link))
    for sh in sheet_link:
        df=\
        pd.read_csv('/home/ubuntu/bodhiai/question_data/csv/'+sh,error_bad_lines=False )
        ...
        for index,i in enumerate(images):
            try:
                existing_quest = SSCquestions.objects.get(picture = i) 
                print('subject - {},  chapter -\
                      {}'.format(existing_quest.section_category,existing_quest.topic_category))
                choices = existing_quest.choices_set.all().order_by('id')
                for which,ch in enumerate(choices):
                    which = which + 1
                    sheet_correct = right_answer[index]
                    if int(which) == int(sheet_correct):
                        ch.predicament = 'Correct'
                        ch.explanationPicture = exp[index]
                        ch.save()
                        print('found right ans - {} -{}'.format(which,sheet_correct))
                    else:
                        ch.predicament = 'Wrong'
                        if ch.explanationPicture != '' or ch.explanationPicture\
                        != None:
                            ch.explanationPicture = ''
                        print('wrong choice - {} -{}'.format(which,sheet_correct))
                        ch.save()
            except Exception as e:
                print(str(e))
...
def create_fake_users():
    df = pd.read_csv('/home/ubuntu/bodhiai/database.csv')
    names = df['NAME']
    print('found names in database')
    for index,na in enumerate(names):
        if index == 20000:
            break
        second_part_username = random.randint(0,999)
        final_username = str(na).lower()+ str(second_part_username)
        new_user =\
        User.objects.create_user(username=final_username,email='info@bodhiai.in',password='12345678')
        school = School.objects.get(name='BodhiAI')
        kl = klass.objects.get(school = school,name='SSC')
        student = Student()
        student.studentuser = new_user
        student.name = na
        student.school = school
        student.klass = kl
        student.pincode = int(12345)
        student.save()
        my_group = Group.objects.get(name='Students')
        my_group.user_set.add(new_user)
        promo_code = PromoCode()
        promo_code.student = student
        promo_code.phone = '9521448254'
        promo_code.date = timezone.now()
        if index % 3 == 0:
            promo_code.code = 'dastak'
            pr_institute = PromoInstitute.objects.get(name='Dastak Coaching Institute')
            promo_code.institute = pr_institute
        else:
            promo_code.code = 'cgs'
            pr_institute = PromoInstitute.objects.get(name='CGS Coaching')
            promo_code.institute = pr_institute
        ...
        subGenInte =\
        Subject(name="General-Intelligence",student=student,teacher=teacher)
        subGenInte.save()
        subMaths =\
        Subject(name="Quantitative-Analysis",student=student,teacher=teacher)
        subEnglish = Subject(name="English",student=student,teacher=teacher)
        subGenKnow =\
        Subject(name="General-Knowledge",student=student,teacher=teacher)
        subMaths.save()
        subEnglish.save()
        subGenKnow.save()
        all_tests = SSCKlassTest.objects.filter(creator = teacher.teacheruser,klas
...
@shared_task
def fakes_users_test():
    students = Student.objects.filter(pincode = int(12345))
    number_students = len(students)
    #seventy_percent = (number_student/10) * 7
    #twenty_percent = (number_student/10) * 2
    #one_percent = (number_student/ 10) * 1
    #nine_percent = (number_student/10) * 9
    #seventy_students = []
    #twenty_student = []
    #one_students = []
    #for st_ind, st in students:
    #    if st_ind != seventy_percent:
    #        seventy_students.append(st)

    time_quest = [5,12,14,20,30,70,80,35,32,9,25,28]
    for stud in students:
        tests = SSCKlassTest.objects.filter(testTakers = stud)
        for test_index, test_obj in enumerate(tests):
            try:
                already_taken = SSCOnlineMarks.objects.get(test =
                                                           test_obj,student =
                                                           stud)
                continue
            except:
                if test_index == 2:
                    break
                questions = test_obj.sscquestions_set.all()
                overall_ans = []
                time_all_ids = []
                for i in questions:
                    quest_ids = []
                    quest_ids.append(i.id)
                    choices = i.choices_set.all()
                    all_ch = []
                    for ch in choices:
                        all_ch.append(ch.id)
                    choice = random.choice(all_ch)
                    quest_ids.append(choice)
                    time_per_qu = random.choice(time_quest)
                    quest_ids.append(time_per_qu)
                    time_all_ids.append(time_per_qu)
                    overall_ans.append(quest_ids)
                ...
                outer = overall_ans
                whole_test_time = 0
                for i in time_all_ids:
                    whole_test_time += i

                tot = 0
                total_time = whole_test_time
                me = Studs(stud.studentuser)
                test = SSCKlassTest.objects.get(id = test_id)
                subject = test.sub
                online_marks = SSCOnlineMarks()
                online_marks.test = test
                ...
                for sub in subjects_in_test_list:
                    try:
                        saveSubjectWiseAccuracyCache.delay(stud.studentuser.id,test_id,sub)
                    except Exception as e:
                        print(str(e))
                for sub in subjects_in_test_list:
                    try:
                        saveSubjectWiseAccuracyRanking.delay(stud.studentuser.id,sub)
                    except Exception as e:
                        print('error in subject accuracy rank {}'.format(str(e)))
                for sub in subjects_in_test_list:
                    try:
                        saveChapterWiseAccuracyCache.delay(stud.studentuser.id,test_id,sub)
                    except Exception as e:
                        print('error in chapter accuracy rank {}'.format(str(e)))
 
@shared_task
def delete_chapter_ranking():
    chapter_ranking = ChapterRank.objects.all()
    for i in chapter_ranking:
        i.delete()



@shared_task
def send_notification_challenged(stud_id,score):
    me_student = Student.objects.get(id = stud_id)
    challenged = StudentChallenge.objects.filter(studentOne = me_student)
    all_challenged_list = []
    for i in challenged:
        all_challenged_list.append(i)
    from_challenged = StudentChallenge.objects.filter(studentTwo = me_student)
    for i in from_challenged:
        all_challenged_list.append(i)
    if len(all_challenged_list) == 0:
        return
    else:
        for i in all_challenged_list:
            if i.studentOne.id != stud_id:
                tokenObj = StudentExpoToken.objects.get(student= i.studentOne)
                tok = tokenObj.token
                name = i.studentOne.name
                username = i.studentOne.studentuser.username
                stud_detail =\
                StudentDetails.objects.get(student=i.studentOne.studentuser) 
                photo = stud_detail.photo
            else:
                tokenObj = StudentExpoToken.objects.get(student= i.studentTwo)
                tok = tokenObj.token
                username = i.studentTwo.studentuser.username
                stud_detail =\
                StudentDetails.objects.get(student=i.studentTwo.studentuser) 
                photo = stud_detail.photo
                name = i.studentTwo.name

            message =\
            """{} your challenger just scored {}. Beat him !!""".format(name,score)
            title = "{} ko haraayein.".format(name)
            in_id = {'username':username,'name':name,'photo':photo}
            custom_data = {'activity':'challenge','id':in_id}
            sendExpoNotification2.delay(tok,message,title,custom_data)


@shared_task
def replicate_questions_course():
    questions = SSCquestions.objects.filter(section_category='Physics_IIT')
    for i in questions:
        new_quest = i
        new_quest.pk = None
        new_quest.section_category = 'Physics_NEET'
        new_quest.save()
        print('saved')
...
def track_progress_cache(student_id,subject,marks_id):
    ...
    subject_chapter_dict = {}
    for quest in marks.test.sscquestions_set.all():
        subject = quest.section_category
        chapter = quest.topic_category
        whole_name = '{}_{}'.format(subject,chapter)
        if whole_name in subject_chapter_dict:
            subject_chapter_dict[whole_name] =\
            subject_chapter_dict[whole_name].append(quest.id)
        else:
            subject_chapter_dict[whole_name] = [quest.id]
    for k,v in subject_chapter_dict.items():
        subject = k.split('_')[0]
        chapter = k.split('_')[1]

        try:
            progressCache = StudentProgressChapterCache.objects.get(student =
                                                               student,subject =
                                                               subject,chapter =
                                                               chap)
            ...
            wrong_time = []
            for quest_id in v:
                if quest_id in marks.skippedAnswers:
                    skipped += 1
                    break
                quest = SSCquestions.objects.get(id = quest_id)
                answered = SSCansweredQuestion.objects.get(onlineMarks =
                ...
                opts = quest.choices_set.all()
                for opt in opts:
                    if opt in marks.rightAnswers:
                        right += 1
                        marks += quest.max_marks
                        right_time.append(answered.time)
                        break
                    if opt in marks.wrongAnswers:
                        wrong += 1
                        marks -= quest.negative_marks
                        wrong_time.append(answered.time)
                        break
    ...
    # saving lists to cached progress
            ...
            progress.save()
        except StudentProgressChapterCache.DoesNotExist:
            right = 0
            wrong = 0
            skipped = 0
            chapter_mark = 0
...
@shared_task
def updateStudyPlan(student_id,marks_id):
    student = Student.objects.get(id = student_id)
    study_plan = StudyPlan.objects.get(student=student)
    marks = SSCOnlineMarks.objects.get(id = marks_id)
    right = len(marks.rightAnswers)
    wrong = len(marks.wrongAnswers)
    skipped = len(marks.skippedAnswers) 
    attempted = right + wrong
    for ra in marks.rightAnswers:
        quest = SSCquestions.objects.get(choices__id = ra)
        sub = quest.section_category
        chap = quest.topic_category
        subject_chapter =\
        SubjectChapters.objects.get(subject=sub,code=float(chap))
        chapter_plan =\
        ChapterPlan.objects.get(studyPlan=study_plan,chapter=subject_chapter)
        chapter_plan.solved += 1
        chapter_plan.to_solve -= 1
        chapter_plan.save()
